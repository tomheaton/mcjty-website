...
export const generalSpawnKeywords = z.object({
    ...
    persecond: z.optional(z.any()).refine((v) => v === undefined, "'persecond' only works for spawner.json"),
    attempts: z.optional(z.any()).refine((v) => v === undefined, "'attempts' only works for spawner.json"),
    amount: z.optional(z.any()).refine((v) => v === undefined, "'amount' only works for spawner.json"),
    conditions: z.optional(z.any()).refine((v) => v === undefined, "'conditions' only works for spawner.json"),
  },
);
...
export const spawnerSchema = z.array(
  z
    .object({
      ...
      persecond: z.number(),
      result: z.optional(z.any()).refine((v) => v === undefined, "'result' only works for spawn.json"),
      amount: z
        .object({
