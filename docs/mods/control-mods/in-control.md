...
* `spawn.json`: with this file you can block spawning of certain creatures under certain conditions. In addition, when a spawn is allowed you can also alter some properties of the mob like maximum health and others
...
In this section the rules in spawn.json and `summonaid.json` are explained.
Every rule is basically a set of `criteria` and a set of `actions`.
Note that in the case of summonaid.json the affected mob will most likely be a zombie.
...
In this section the rules in `potentialspawn.json` are explained.
Every rule is basically a set of criteria and then a list of additional mobs to spawn under those criteria.
The following criteria as explained from spawn.json can also be used here: mintime, maxtime, minlight, maxlight, minheight, maxheight, minspawndist, maxspawndist, random, structure, mindifficulty, maxdifficulty, seesky, weather, tempcategory, difficulty, block, biome, and dimension.
mincount and maxcount are also supported, but they only work in the format `<amount>,<mob>`. i.e. you have to specify a mob.
...
Here we explain how you can control the loot given by mobs when they are killed.
The following criteria from `spawn.json` can also be used here: mintime, maxtime, minlight, maxlight, minheight, maxheight, minspawndist, maxspawndist, random, mindifficulty, maxdifficulty, passive, hostile, seesky, weather, tempcategory, structure, difficulty, block, biome, and dimension.
In addition, the following criteria are specific for loot:
...
```json title="potentialspawn.json"
[
  {
    "dimension": 0,
